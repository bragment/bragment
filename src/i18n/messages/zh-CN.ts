export default {
  'auth.continueWithGithub': 'GitHub 登录',
  'auth.email': '邮箱',
  'auth.getPasscode': '获取验证码',
  'auth.getPasscodeFirst': '请先获取验证码',
  'auth.githubEmailTaken': '该 GitHub 帐号邮箱已注册，\n请用邮箱登录',
  'auth.invalidEmail': '邮箱无效',
  'auth.invalidGithubCode': 'GitHub 授权码无效',
  'auth.invalidPasscode': '验证码无效',
  'auth.passcode': '验证码',
  'auth.passcodeMailSent': '验证码已发往邮箱',
  'auth.rememberMe': '自动登录',
  'auth.requiredEmail': '请输入邮箱',
  'auth.signInOrSignUp': '登录 / 注册',
  'auth.signOut': '退出',
  'auth.waitingSeconds': '等待 {seconds} 秒',
  'auth.yourEmail': '你的邮箱',
  'common.clickAndRetry': '点击重试',
  'common.confirm': '确定',
  'common.contain': '包含',
  'common.create': '创建',
  'common.delete': '删除',
  'common.edit': '编辑',
  'common.equal': '等于',
  'common.errorOccurred': '🧰 发生错误...',
  'common.home': '主页',
  'common.networkError': '网络出错，请稍后再试',
  'common.remove': '移除',
  'common.required': '必填',
  'common.save': '保存',
  'common.search': '搜索...',
  'common.setting': '设置',
  'common.tooManyRequests': '请求过多',
  'common.underConstruction': '🚧 正在施工...',
  'dataField.addOption': '添加选项',
  'dataField.advanced': '高级字段',
  'dataField.asType': '作为',
  'dataField.basic': '基础字段',
  'dataField.builtin': '内置字段',
  'dataField.date': '日期',
  'dataField.dateTime': '日期时间',
  'dataField.description': '描述',
  'dataField.httpLink': 'Http 链接',
  'dataField.image': '图片',
  'dataField.multipleLineText': '多行文本',
  'dataField.multipleSelect': '多选',
  'dataField.number': '数字',
  'dataField.option': '选项',
  'dataField.path': '路径',
  'dataField.plain': '文本',
  'dataField.provider': '来源',
  'dataField.resolve': '解析',
  'dataField.resolver': '解析器',
  'dataField.selectAFieldType': '选择字段类型',
  'dataField.singleLineText': '单行文本',
  'dataField.singleSelect': '单选',
  'dataField.title': '标题',
  'dataForm.allFieldsUsed': '已用上全部字段',
  'dataView.addData': '添加数据',
  'dataView.addField': '添加字段',
  'dataView.addFilter': '添加过滤',
  'dataView.addSorter': '添加排序',
  'dataView.ascending': '升序',
  'dataView.board': '看板',
  'dataView.descending': '降序',
  'dataView.fieldVisibility': '显示字段',
  'dataView.filter': '过滤',
  'dataView.hide': '隐藏',
  'dataView.list': '列表',
  'dataView.mainField': '主字段',
  'dataView.mainFieldVisibleFirst': '主字段总是最先显示',
  'dataView.moveLeft': '左移',
  'dataView.moveRight': '右移',
  'dataView.pin': '固定',
  'dataView.setAsMainField': '设置为主字段',
  'dataView.sorting': '排序',
  'dataView.table': '表格',
  'dataView.unpin': '取消固定',
  'project.addView': '添加视图',
  'project.createData': '创建数据',
  'project.createForm': '创建表单',
  'project.createModel': '创建数据源',
  'project.description': '项目描述',
  'project.editForm': '编辑表单',
  'project.existingFieldTitle': '字段名称已存在！',
  'project.field': '字段',
  'project.fieldTitle': '字段名称',
  'project.fieldType': '字段类型',
  'project.form': '表单',
  'project.formTitle': '表单名称',
  'project.getModelStarted': '从创建一个新的数据源开始吧！',
  'project.hideOtherFields': '隐藏其他字段',
  'project.model': '数据源',
  'project.modelTitle': '数据源名称',
  'project.myProjects': '我的项目',
  'project.newForm': '新表单',
  'project.noFields': '暂无字段',
  'project.noModels': '暂无数据源',
  'project.noViews': '暂无视图',
  'project.noViewsFound': '没有找到视图',
  'project.showOtherFields': '显示其他字段',
  'project.title': '项目名称',
  'workspace.allProject': '所有项目',
  'workspace.createProject': '新建项目',
  'workspace.createWorkspace': '新建工作区',
  'workspace.getProjectStarted': '从创建一个新的项目开始吧！',
  'workspace.member': '成员',
  'workspace.memberList': '成员列表',
  'workspace.members': '成员',
  'workspace.myWorkspaces': '我的工作区',
  'workspace.noProjects': '暂无项目',
  'workspace.noWorkspacesFound': '没有找到工作区',
  'workspace.owner': '管理员',
  'workspace.projectList': '项目列表',
  'workspace.projects': '项目',
  'workspace.setAsMainWorkspace': '设置为主工作区',
  'workspace.switchTo': '切换到',
  'workspace.title': '工作区名称',
};
